...
import com.arcsoft.arcfacedemo.util.FileUtils;
...
//                }
//            }
            faceList = FileUtils.getInstance().readContentList(new File(Constants.faceLibPath + "/" + Constants.faceLib) , "csv");
            int count = 0;
            for (String face : faceList){
                faceRegisterInfoList.add(new FaceRegisterInfo(Base64.decode(face , Base64.DEFAULT) , "user" + count));
                count ++;
            }
        }
    }
    ...
    public int getFaceNumber() {
        synchronized (this) {
            File file = new File(Constants.faceLibPath + Constants.faceLib);
            if(file.exists()){
                int size = FileUtils.getInstance().readContentList(file , "csv").size() - 1;
                return size;
            }
            return 0;
        }
            ...
            if (code == ErrorInfo.MOK && faceInfoList.size() > 0) {
                ...
                code = faceEngine.extractFaceFeature(nv21, width, height, FaceEngine.CP_PAF_NV21, faceInfoList.get(0), faceFeature);
                    ...
                    if (code == ErrorInfo.MOK) {
                        ...
                        faceRegisterInfoList.add(new FaceRegisterInfo(faceFeature.getFeatureData(), "user" + faceRegisterInfoList.size()));
                        FileUtils.getInstance().writeTxtToFile(Base64.encodeToString(faceFeature.getFeatureData() , Base64.DEFAULT) , Constants.faceLibPath , Constants.faceLib);
                        return true;
                    }
            }
            return false;
