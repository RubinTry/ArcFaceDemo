...
public class DrawHelper {
    ...
    public static void drawFaceRect(Canvas canvas, DrawInfo drawInfo, int color, int faceRectThickness) {
        ...
        if (drawInfo.getName() == null) {
            ...
            String str = (drawInfo.getSex() == GenderInfo.MALE ? "男" : (drawInfo.getSex() == GenderInfo.FEMALE ? "女" : "未知"))
                    + ","
                    + (drawInfo.getAge() == AgeInfo.UNKNOWN_AGE ? "未知" : drawInfo.getAge())
                    + ","
                    + (drawInfo.getLiveness() == LivenessInfo.ALIVE ? "活体" : (drawInfo.getLiveness() == LivenessInfo.NOT_ALIVE ? "非活体" : "未知"));
